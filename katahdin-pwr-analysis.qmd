...
zbdata <- function(x1, x2, r0, size, a0, a1, a2, b1, b2) {
    ...
    zdata$mubin <- logitlink(a0 + a1 * x1 - a2 * x1^2 + b1 * x2 - b2 * x2^2, inverse = TRUE)
...
# vectors of params for different scenarios 

splst_u <- c(2, -1, 2, 2, 2, 2)
splst_r <- c(0, -1, 2, 2, 2, 2)
gnlst_u <- c(2, -1, 2, 2, 2, 2) # change
gnlst_r <- c(0, -1, 2, 2, 2, 2) # change

names(splst_u) <- names(splst_u) <- names(splst_u) <- names(splst_u) <- c("r0", "a0", "a1", "a2", "b1", "b2")

# data for strong climate response to strata, climate specialist, ubiquitous 
dat_strg_splst_u <- zbdata(x1 =, x2=, r0=, size=, a0=, a1=, a2=, b1=, b2=)
